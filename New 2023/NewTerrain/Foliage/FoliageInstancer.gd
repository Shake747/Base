...
@onready var terrain: Terrain3D = $"../Terrain"
@onready var noise_gen: Node = $"../Terrain/NoiseGenerator"


var terrain_ready: bool = false
var height_keeper: float
#MULTITHREAD VARS
...
@export var collider_coverage_dist : float = 50.0
...
func create_multimesh():
	...
	v_scale = terrain.map_height_scale
	...
	timer = Timer.new()
	add_child(timer) #used to have get_parent().add_child - kept erroring saying parent was busy
...
func _update():
	...
	#i_spac_dq = instance spacing * instansance spacing (160 right now)
...
func get_heightmap_y(x, z):
	...
	return color.r * terrain_height * v_scale * 2
...
func generate_subset():
	for i in range(instance_amount):
		...
		if t.origin.distance_squared_to(player_node.global_position) < pow(collider_coverage_dist,2):
			print("squared to player",t.origin.distance_squared_to(player_node.global_position))
			print("power to cov dist",pow(collider_coverage_dist,2))
